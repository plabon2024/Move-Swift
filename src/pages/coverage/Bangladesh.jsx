...
const Bangladesh = ({ serviceCenters }) => {
  ...
  return (
    <div className="h-[800px] w-full rounded-lg overflow-hidden shadow-lg relative">
      <form
        ...
        className="absolute top-4 left-1/2 transform -translate-x-1/2 z-[1000] rounded-lg  flex backdrop-blur-3xl"
      >
        <input
          type="text"
          placeholder="Search district..."
          className="flex-1 px-4 py-2 focus:border-none focus:outline-none"
          value={searchText}
          onChange={(e) => setSearchText(e.target.value)}
        />
        <button type="submit" className="bg-slate-400 text-black px-4 py-2 rounded-r-md ">
          Go
        </button>
      </form>
